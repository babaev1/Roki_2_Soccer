...
try:
    if SIMULATION == 5:
        ...
        for i in range(robots_Number):
            ...
            if i == 0 or i == 3:
                second_pressed_button = 'rotation_right'   # side_step_left, side_step_right, short_run, rotation_right, spot_run, start_later
                m.append(Player('test_walk', second_pressed_button, glob[i], motion[i], local[i])) 
            if i == 1:
